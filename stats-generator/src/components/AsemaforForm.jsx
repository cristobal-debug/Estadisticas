...
import {
  ...
  PaginasGrid, FuentesGrid, TopPostsGrid, NumField
} from './FormFields'

export default function AsemaforForm({ data, onChange }) {
  const set = (path, val) => {
  ...
  const setArr = (section, field, idx, val, subfield = null) => {
    const updated = JSON.parse(JSON.stringify(data))
    if (subfield) {
      updated[section][idx][subfield] = val
    } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
      ...
      {/* WEB */}
      <div className="card">
        <SectionTitle icon="🌐" title="Web — asemafor.cl" color="var(--cyan)" />
        ...
        <PaisesGrid
          ...
          onChange={(i, v) => {
            const updated = JSON.parse(JSON.stringify(data))
            const key = 'porcentaje' in updated.web.paises[i] ? 'porcentaje' : 'valor'
            updated.web.paises[i][key] = v
            onChange(updated)
          }}
          onChangeName={(i, v) => {
            const updated = JSON.parse(JSON.stringify(data))
          ...
          onAdd={() => {
            const updated = JSON.parse(JSON.stringify(data))
            updated.web.paises.push({ nombre: '', porcentaje: '' })
            onChange(updated)
          }}
          onRemove={i => {
            const updated = JSON.parse(JSON.stringify(data))
      ...
      {/* INSTAGRAM */}
      <div className="card">
        <SectionTitle icon="📸" title="Instagram — @asemafor" color="var(--magenta)" />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
          <div>
            ...
            <input type="number" value={data.instagram.seguidoresPct} onChange={e => set('instagram.seguidoresPct', e.target.value)} placeholder="77" min="0" max="100" />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>No seguidores %</label>
            <input type="number" value={data.instagram.noSeguidoresPct} onChange={e => set('instagram.noSeguidoresPct', e.target.value)} placeholder="23" min="0" max="100" />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>Publicaciones % (alcance)</label>
            <input type="number" value={data.instagram.publicacionesPct} onChange={e => set('instagram.publicacionesPct', e.target.value)} placeholder="89" min="0" max="100" />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>Reel % (alcance)</label>
            <input type="number" value={data.instagram.reelPct} onChange={e => set('instagram.reelPct', e.target.value)} placeholder="11" min="0" max="100" />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>Interacc. Seguidores %</label>
            <input type="number" value={data.instagram.interaccionesSegPct} onChange={e => set('instagram.interaccionesSegPct', e.target.value)} placeholder="83" min="0" max="100" />
          </div>
          <div>
            <label style={{ fontSize: 11, color: 'var(--muted)', display: 'block', marginBottom: 3 }}>Interacc. No seg. %</label>
            <input type="number" value={data.instagram.interaccionesNoSegPct} onChange={e => set('instagram.interaccionesNoSegPct', e.target.value)} placeholder="17" min="0" max="100" />
          </div>
        </div>
